...
<div id="view-daily" class="view active">
  ...
  <div id="dp-review" class="phase" style="display:none">
    ...
    <h2>Self-check</h2>
    <p class="sub" style="margin-bottom:1rem">Before seeing the model answer — how did you do?</p>
    ...
    <div class="card">
      <span class="slabel">Your answer</span>
      <div id="dp-ans-display" style="font-size:13px;color:var(--ink-ghost);line-height:1.7;white-space:pre-wrap;background:var(--surface);padding:12px;border-radius:var(--r)"></div>
    </div>

    <div class="card">
      <button onclick="dpToggleModel()" style="width:100%;background:none;border:1px solid var(--border-med);border-radius:var(--r);font-family:'DM Sans',sans-serif;font-size:13px;color:var(--ink-dim);cursor:pointer;padding:10px 14px;text-align:left;display:flex;justify-content:space-between;align-items:center;transition:background 0.15s">
        <span>Compare with model answer</span>
        <span style="color:var(--ink-ghost);font-size:12px">from drill script &#9662;</span>
      </button>
      <div id="dp-model-answer" style="display:none;margin-top:0.75rem">
        <div class="ans-box" id="dp-model-text"></div>
        <div class="insight" id="dp-model-insight"></div>
      </div>
    </div>

    <div id="dp-ai-score-area" style="display:none" class="card">
    ...
    <div class="btn-row">
      <button class="btn btn-primary" onclick="dpNext()">Next &rarr;</button>
      <button class="btn" onclick="dpRetry()">Try again</button>
      <button class="btn-ghost" onclick="dpHome()">Home</button>
    </div>
  </div>
  ...
  <div id="dp-summary" class="phase" style="display:none">
    ...
    <div class="btn-row">
      <button class="btn btn-primary" onclick="dpHome()">Back to home</button>
    </div>
  </div>
...
function dpShowQ(){
  ...
  const total=DP.queue.length;
  $('dp-q-num').textContent=total>1?`Question ${DP.qIdx+1} of ${total}`:'';
  $('dp-q-text').textContent=q.q;
  $('dp-prog').style.width=total>1?((DP.qIdx/total)*100)+'%':'0%';
...
function dpRunChecklist(ans){
  ...
  const pct=((DP.qIdx+1)/Math.min(DP.queue.length,7))*100;
  $('dp-prog-rv').style.width=pct+'%';

  // AI score button if Claude key set
  const ck=localStorage.getItem('mb_dk');
  if(ck){
...
function dpToggleModel(){
  const d=$('dp-model-answer');
...
function dpSkip(){
  DP.answers.push({q:DP.queue[DP.qIdx].q,a:'[skipped]',t:0});
  dpNext();
}
...
function dpShowPhase(id){
  ['dp-home','dp-question','dp-review','dp-summary'].forEach(p=>{
