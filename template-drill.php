...
<nav>
  <a href="/" class="nav-logo">Manuel Becerra<span class="nav-dot">.</span></a>
  <div class="nav-right">
...
function startRec(){
  ...
  S.recog.onresult=e=>{
    ...
    S.finalTranscript=finalText.trim();
    S.transcript=[S.finalTranscript,interimText].filter(Boolean).join(' ').trim();
...
async function submitAnswer(){
  const ans=S.input==='voice'?S.transcript.trim():$('text-answer').value.trim();
...
async function scoreFeedback(question,answer,timeSec){
  ...
  try{
    ...
    $('fb-stream').innerHTML=`<div class="feedback-body">${S.lang==='de'?'Fehler. API-Key prüfen.':'Could not score. Check API key.'}</div>`;
    $('fb-btns').style.display='flex';
    $('retry-btn').textContent=S.lang==='de'?T.de.retry:T.en.retry;
...
function restartSession(){S.cq=0;S.answers=[];S.scores=[];S.selfR=[];S.transcript='';S.wk={own:0,met:0,hed:0,str:0,len:0};updateStats();showP('ph-setup');}
